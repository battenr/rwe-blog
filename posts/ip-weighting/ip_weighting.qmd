...
cbind(mean(df.tramp$age), sd(df.tramp$age)) # age
rwetasks::count_percent(df.tramp, sex) # sex
rwetasks::count_percent(df.tramp, t_shirt) # t-shirt
rwetasks::count_percent(df.tramp, brain_freeze) # brain-freeze
cbind(mean(df.tramp$age), sd(df.tramp$time_standing)) # time standing
rwetasks::count_percent(df.tramp, headache) # headache
...
Now we are ready to estimate some weights! We will estimate the weights using a logistic regression with the following variables: age, sex, t-shirt color and time standing. Using that model we will calculate the weights, for trampoline jumpers as:\
$$
...
df = df %>% 
  dplyr::mutate(
    trampoline = dplyr::case_when(
      group == "trampoline" ~ 1, 
      group == "chair" ~ 0 
    )
  )

mod.fit <- stats::glm(formula = trampoline ~ age + as.factor(sex) + as.factor(t_shirt) + time_standing + I(age ^ 2) + I(time_standing ^ 2), 
...
df.weights <- df %>% 
  dplyr::mutate(
    ps = dplyr::case_when(
      trampoline == 1 ~ predict(mod.fit, 
                                type = "response"),
      trampoline == 0 ~ (1 - predict(mod.fit, 
                                    type = "response"))
    ),
    ip_weights = 1/ps,
    half_ipw = 0.5/ps
...
If we look back to our example, we can examine the summary statistics of these weights. A couple of things jump out at first:

1.  The mean does not equal 2

2.  The minimum weight is 1, which seems okay, however the maximum weight is ***23!!***

Something seems off about this. Now there are a few options. One would be to create a pseudo-population similar to what we have done, expect use 0.5 for the numerator. That is, the unconditional probability of being a trampoline jumper is 0.5 and the unconditional probability of not being a trampoline jumper is 0.5. In this scenario the pseudo-population would be the same size as the study population, and would be equal to if we used $\frac{1}{f(A|L}$ but divided all the weights by 2 [@hernanwhatif, pp.153]. We can write this more generally
...
where $p$ is the unconditional probability of treatment. Note: $0< p \leq 1$, whereas $f[A|L]$ is the probability of treatment based on covariates L. An alternative is for different people to have different probabilities [@hernanwhatif, pp. 153]. A common choice is to use $Pr[A = 1]$ for $p$ in the treated and $Pr[A=0]$ for $p$ in the untreated. $Pr$ in this case would just be the proportion. Using our example again, $Pr[A = \text{trampoline jumpers}] = \frac{260}{729} = 0.357$ and for the not trampoline jumpers, $Pr[A = \text{not trampoline jumper}] = \frac{469}{729} = 0.643$. If we use these values for our example:
...
df.weights <- df %>% 
  dplyr::mutate(
    ...
    ip_weights = dplyr::case_when(
      trampoline == 1 ~ 0.357/ps,
      trampoline == 0 ~ 0.643/ps
  )
  )
...
This is notably different from when we used $\frac{1}{f[A|L]}$. Now the weights range from 0.357 to 14.8 whereas before they ranged from 1 to 23.0. Not only that but now in the pseudo-population, the ratio of trampoline jumpers to not trampoline jumpers is kept. The stabilizing factor, $f[A]$, is responsible for the narrower range [@hernanwhatif, pp. 153]. Weights that use the stabilizng factor are referred to as stabilized weights.
...
mod.intercept <- stats::glm(formula = trampoline ~ 1, 
                      family = binomial(link = "logit"),
                      data = df)
...
df.weights <- df %>% 
  dplyr::mutate(
    ...
    ip_weights = ps.numerator/ps.denominator
  )
  
summary(df.weights$ip_weights)
```

Now we have weights with a mean further away from zero however with a lower maximum weight. There is still one more thing that has to be checked whenever using these cases: the positivity assumption! We have to ensure that all participants have a greater than 0 probabilty of being a trampoline jumper! After checking this, we are confident that the positivity assumption is met.

Now which method would we prefer for estimating the weights? Well this comes down to opinion. My personal preference is to use the weights estimated using regression for both the numerator and denominator.
...
Okay cool, so what now? Glad you asked! Well now you can go freely with your new knowledge and re-weight away!
